...
// verify the presence of Manufacturer  (HARD CODE because This asset  has sync In through application)
	...
	// select Temperature option n click ok to navigate CycleSelectionPage

	public RW_CycleSelectionPage Select_Temperature_ClickOK() throws IOException {
		WebElement Temperature = driver.findElementByName("Temperature");
		clickOn(Temperature);
		WebElement ok_btn = driver.findElementByAccessibilityId("_sensorTypeSelectionOKButton");
		clickOn(ok_btn);
